...
var SyntaxHighlighter = function() { 
	...
	var sh = {
		...
		toolbar: {
			...
			{ 
				var html = '<div id="' + highlighter.params.toolbarContainerId + '_' + highlighter.id + '" class="toolbar">',
			...
			items : {
				...
				help: {
					execute: function(highlighter)
					{ 
						var wnd = popup('', '_blank', 500, 350, 'scrollbars=0'),
							doc = wnd.document
							;
		 ...
		 * 							are highlighted.
		 */ 
		highlight: function(globalParams)
		{
			...
			var highlighter = null,
				...
				var element = document.getElementById(globalParams.codeInputTextarea);
				
				for(i=0;i<element.attributes.length;i++)
				{
					if(element.attributes[i].nodeName==globalParams.custSettingsVar)
					{
						settings = element.attributes[i].value;
						break;
					}
				}					
				
				var	target = element,
					params = merge(globalParams, parseParams(settings) || {}),
				...
				var precode 	= document.createElement('pre'),				
					outputDiv  	= document.getElementById(globalParams.htmlOutputDivId),
					Previewdiv 	= document.getElementById(globalParams.previewDivId),
					formattedHtml = highlighter.getDiv(code).innerHTML;
				
				
				outputDiv.innerHTML = safe_tags(formattedHtml);	
				Previewdiv.innerHTML = formattedHtml;	
				precode.innerHTML = safe_tagsDeep(safe_tags(highlighter.formattedCode));				
				outputDiv.innerHTML += '<br/><br/>' + '&lt;pre id="presourcecode_'+highlighter.id+'" class="displaysourcecode" &gt;' + precode.outerHTML + '&lt;/pre&gt;' ;	
...
//				target.parentNode.replaceChild(element, target);
		...
		{ 
			var presourcecode 			=(typeof gArgs.presourcecode !=='undefined')? gArgs.presourcecode : "presourcecode",	
				...
				previewDivId			=(typeof gArgs.previewDivId !=='undefined')? gArgs.previewDivId : "PreviewSect",	
				htmlOutputDivId			=(typeof gArgs.htmlOutputDivId !=='undefined')? gArgs.htmlOutputDivId : "outputhtml",
				codeInputTextarea		=(typeof gArgs.codeInputTextarea !=='undefined')? gArgs.codeInputTextarea : "highlightcode"	
				;
			...
			var tempParams ={
								...
								"custSettingsVar":custSettingsVar,
								"previewDivId":previewDivId,
								"htmlOutputDivId":htmlOutputDivId,
								"codeInputTextarea":codeInputTextarea
							},		
				
				params = merge(gArgs, tempParams);		
	 ...
	 */
	function quickCodeHandler(e,hId)
	{
	...
	*/
		...
		attachEvent(textarea, 'blur', function(e,id)
		{
	...
	sh.Highlighter.prototype = {
						...
						+ '<tbody>'
							...
							+ '<tr>'
								...
								+ '<td id="' + this.params.codeContainerId + '_' + this.id + '" class="code">'
									+ '<div id="' + this.params.containerId + '_' + this.id + '" class="container">'
										+ html
									+ '</div>'
								+ '</td>'
							+ '</tr>'							
		...
		{ 
			...
			//	attachEvent(findElement(div, '.toolbar'), 'click', sh.toolbar.handler);				
				var node = findElement(div, '#'+this.params.toolbarContainerId+'_'+this.id);
				attachEvent(node, 'click', sh.toolbar.handler,undefined,this.id);				
			}
			
			if (this.getParam('quick-code'))
			{
			//	attachEvent(findElement(div, '.code'), 'dblclick', quickCodeHandler);
				var node = findElement(div, '#'+this.params.codeContainerId+'_'+this.id);
				attachEvent(node, 'dblclick', quickCodeHandler,undefined,this.id);					
			}
